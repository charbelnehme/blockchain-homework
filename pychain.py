...
@dataclass
class Block:
    record: Record # Re-naming the 'data' attribute to 'record', and setting the data type to 'Record'.
    creator_id: int
    prev_hash: str = "0"
...
# Update `new_block` so that `Block` consists of an attribute named `record`, which is set equal to a `Record` that contains the `sender`, `receiver`, and `amount` values. The updated `Block`should also include the attributes for `creator_id` and `prev_hash`.

# Part 3a: The following `input_data` variable was removed from the Streamlit interface.
...
# Test the blockchain validation process by using your `PyChain` ledger.